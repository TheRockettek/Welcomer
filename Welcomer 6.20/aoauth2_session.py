...
class AOAuth2Session(aiohttp.ClientSession):
    ...
    def __init__(self, client_id=None, client=None, auto_refresh_url=None,
        ...
        """
        super(AOAuth2Session, self).__init__(**kwargs)
    ...
    async def fetch_token(self, token_url, code=None, authorization_response=None,
            body='', auth=None, username=None, password=None, method='POST',
            timeout=None, headers=None, proxy=None,
        ...
        :param proxy: The `proxy` argument is passed onto `requests`.
        ...
        if method.upper() == 'POST':
            r = await self.request("POST", token_url, data=dict(urldecode(body)),
                timeout=timeout, headers=headers, auth=auth,
                proxy=proxy)
            log.debug('Prepared fetch token request body %s', body)
        elif method.upper() == 'GET':
            # if method is not 'POST', switch body to querystring and GET
            r = await self.request("GET", token_url, params=dict(urldecode(body)),
                timeout=timeout, headers=headers, auth=auth,
                proxy=proxy)
        ...
        self._client.parse_request_body_response(await r.text(), scope=self.scope)
    ...
    async def refresh_token(self, token_url, refresh_token=None, body='', auth=None,
                      timeout=None, headers=None, proxy=None, **kwargs):
        ...
        r = await self.request("POST", token_url, data=dict(urldecode(body)), auth=auth,
            timeout=timeout, headers=headers, withhold_token=True, proxy=proxy)
        ...
        self.token = self._client.parse_request_body_response(await r.text(), scope=self.scope)
        if 'refresh_token' not in self.token:
            log.debug('No new refresh token given. Re-using old.')
            self.token['refresh_token'] = refresh_token
        return self.token

    async def request(self, method, url, data=None, headers=None, withhold_token=False,
        ...
        return await super(AOAuth2Session, self).request(method, url,
